...
//#else
        ...
        public void Update(GameTime gameTime)
        {
            ...
            else if (TouchCollection.IsConnected && TouchCollection.Count > 0)
            {
 ...
 // TODO: fix MonoGame GamePadState
...
// TODO: fix MonoGame GamePadState
 //           return new GamePadState(stick, Vector2.Zero, 0f, 0f, buttons.ToArray());
        ...
        {
            return (TouchCollection.IsConnected && TouchCollection.Count > 0 && TouchCollection[0].State == TouchLocationState.Pressed);
            
        }

        public bool IsNewTouchRelease()
        {
            return IsTouchReleased();//TouchCollection.IsConnected && TouchCollection.Count > 0 && TouchCollection[0].State == TouchLocationState.Released &&
                   //TouchCollection[0].TryGetPreviousLocation(out var t) && t.State != TouchLocationState.Released;
        }

        public bool IsTouchPressed()
        {
            return (TouchCollection.IsConnected && TouchCollection.Count > 0 && (TouchCollection[0].State == TouchLocationState.Pressed || TouchCollection[0].State == TouchLocationState.Moved));
        }

        public bool IsTouchReleased()
        {
            return (TouchCollection.IsConnected && TouchCollection.Count > 0 && TouchCollection[0].State == TouchLocationState.Released);
        }
        ...
        {
            return IsNewKeyPress(Keys.Space) || IsNewKeyPress(Keys.Enter) || IsNewButtonPress(Buttons.A) ||
                   IsNewButtonPress(Buttons.Start) || IsNewMouseButtonPress(MouseButtons.LeftButton) || IsNewTouchPress();
        }

        public bool IsMenuPressed()
        {
            return KeyboardState.IsKeyDown(Keys.Space) || KeyboardState.IsKeyDown(Keys.Enter) || GamePadState.IsButtonDown(Buttons.A) ||
                   GamePadState.IsButtonDown(Buttons.Start) || MouseState.LeftButton == ButtonState.Pressed ||
                   IsTouchPressed();
        }

        public bool IsMenuReleased()
        {
            return IsNewKeyRelease(Keys.Space) || IsNewKeyRelease(Keys.Enter) || IsNewButtonRelease(Buttons.A) ||
                   IsNewButtonRelease(Buttons.Start) || IsNewMouseButtonRelease(MouseButtons.LeftButton) || IsNewTouchRelease();
        }
